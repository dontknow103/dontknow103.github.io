...
const ortSession = ref<InferenceSession | null>(null)
    ...
    if (!worker) {
      ...
      worker.onmessage = (e) => {
        if (e.data.type === 'init' && e.data.status === 'success') {
          ...
          loadModel();
        } else if (e.data.type === 'loadModel' && e.data.status === 'success') {
          console.log("Model loaded successfully");
          modelLoaded = true;
        } else if (e.data.type === 'run' && e.data.status === 'success') {
          // Handle inference results
          console.log("Inference completed:", e.data.result);
        } else if (e.data.status === 'error') {
          console.error("Error in Web Worker:", e.data.error);
        }
      };
...
const loadModel = () => {
  if (worker) {
    ...
    worker.postMessage({ type: 'loadModel', modelPath });
  }
};

const runInference = (inputs: any) => {
  if (worker && modelLoaded) {
    worker.postMessage({ type: 'run', feeds: inputs });
  } else {
    console.error("Worker not initialized or model not loaded");
  }
};
...
const predictAudio = async (tokens: number[], tones: number[]): Promise<Float32Array> => {
  ...
  try {
    ...
    const tokenTensor = new Tensor('int64', BigInt64Array.from(tokens.map(BigInt)), [1, tokens.length])
    const toneTensor = new Tensor('int64', BigInt64Array.from(tones.map(BigInt)), [1, tones.length])
    const speakerTensor = new Tensor('int64', BigInt64Array.from(speaker_id.map(BigInt)), [1])
