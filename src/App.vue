...
const excluded_syllables: string[] = ["lo", "din", "<sil>", "lia", "den", "ei", "yo", "o", "ng", "eng", "tei", "dia", "yai"]
...
const resetQuiz = () => {
  ...
  confusionMatrices.value = Array(2).fill(Array(4).fill(Array(4).fill(0)));
...
const predictAudio = async (tokens: number[], tones: number[]): Promise<Float32Array> => {
  ...
  const feeds: InferenceFeeds = {
    speakers: speaker_id,
    tokens: tokens,
    tones: tones,
  };

  return await onnxService.runInference(feeds);
};
